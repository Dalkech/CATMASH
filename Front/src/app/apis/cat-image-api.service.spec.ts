...
class MockInterceptor implements HttpInterceptor {
  intercept(req: HttpRequest<any>, next: HttpHandler) {
    return next.handle(req); // Pass through without modification
  }
}
...
describe('CatImageApiService', () => {
  ...
  it('should fetch cat images', () => {
    ...
    const req = httpMock.expectOne('http://localhost:5181/catimages'); // Adjust URL if needed
  ...
  it('should fetch one cat image with Id', () => {
    ...
    const req = httpMock.expectOne('http://localhost:5181/catimages/1'); // Adjust URL if needed
    expect(req.request.method).toBe('GET');
    req.flush(mockImage);
  });
});
